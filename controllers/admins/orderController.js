        ...
        if (action === 'approve') {
          ...
          try {
            ...
            if (walletError.code === 11000) {
              return res.status(500).json({ message: 'Transaction ID conflict. Please try again.' });
            }
            return res.status(500).json({ message: 'Error processing refund. Please try again.' });
          }
